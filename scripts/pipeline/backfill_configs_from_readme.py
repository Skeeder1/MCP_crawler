...
class ConfigBackfiller:
    ...
    def save_npm_config(self, session, server: Server, config_data: Dict) -> bool:
        ...
        try:
            ...
            # Normalize command to fit constraint (npx, node, npm only)
            raw_command = install_config.get("command", "npx")
            raw_args = install_config.get("args", [])

            # Map non-standard commands to 'npx' and preserve original in args
            if raw_command in ['uvx', 'pip', 'python', 'git', 'docker', 'uv']:
                # Store original command as first arg
                normalized_args = [raw_command] + raw_args
                normalized_command = 'npx'  # Use npx as placeholder
            elif raw_command.startswith('/') or raw_command.startswith('D:') or raw_command.startswith('C:'):
                # Absolute path - normalize to 'node'
                normalized_args = [raw_command] + raw_args
                normalized_command = 'node'
            else:
                normalized_command = raw_command if raw_command in ['npx', 'node', 'npm'] else 'npx'
