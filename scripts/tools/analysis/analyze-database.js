...
if (!fs.existsSync(DB_PATH)) {
    console.error(chalk_1.default.red.bold(`❌ Database not found at: ${DB_PATH}`));
    process.exit(1);
}
...
function getAllGithubInfo() {
    return db.prepare('SELECT * FROM github_info').all();
}
function getAllNpmInfo() {
    return db.prepare('SELECT * FROM npm_info').all();
}
function getAllMcpConfigNpm() {
    return db.prepare('SELECT * FROM mcp_config_npm').all();
}
function getAllMcpConfigDocker() {
    return db.prepare('SELECT * FROM mcp_config_docker').all();
}
function getAllTools() {
    return db.prepare('SELECT * FROM tools').all();
}
// ============================================================================
// Analyzers
// ============================================================================
...
function analyzeActivity() {
    ...
    githubInfos.forEach((info) => {
        ...
        try {
            const lastCommit = (0, date_fns_1.parseISO)(info.github_last_commit);
            const daysSinceCommit = (0, date_fns_1.differenceInDays)(now, lastCommit);
...
function analyzeQuality() {
    ...
    githubInfos.forEach((info) => {
        if (info.has_readme === 1)
            hasReadme++;
        if (info.has_license === 1)
            hasLicense++;
        if (info.has_contributing === 1)
            hasContributing++;
        if (info.has_code_of_conduct === 1)
            hasCodeOfConduct++;
        if (info.is_archived === 1)
            isArchived++;
        if (info.is_disabled === 1)
            isDisabled++;
        if (info.is_fork === 1)
            isFork++;
...
function analyzeCompleteness() {
    ...
    const addReport = (tableName, fieldName, presentCount, totalCount, description) => {
        reports.push({
            tableName,
            fieldName,
            presentCount,
 ...
 */
function generateInsights(configStats, healthScores, activity, popularity, quality) {
    const insights = [];
    const total = configStats.totalServers;
    // Configuration insights
    if (configStats.withNoConfig > total * 0.8) {
        insights.push(`⚠️  ${configStats.withNoConfig} serveurs (${configStats.noConfigPercentage}%) n'ont AUCUNE configuration d'installation (NPM/Docker)`);
    }
    // Health score insights
    if (healthScores.poor > 0) {
        insights.push(`🔴 ${healthScores.poor} serveurs ont un score de santé FAIBLE (<40%)`);
    }
    // Activity insights
    if (activity.lastCommitMoreThan6Months > 0) {
        insights.push(`⏰ ${activity.lastCommitMoreThan6Months} serveurs n'ont PAS eu de commit depuis plus de 6 mois`);
    }
    // Quality insights
    ...
    if (quality.hasLicense < total * 0.8) {
        insights.push(`⚖️  Seulement ${quality.hasLicense} serveurs (${calculatePercentage(quality.hasLicense, total)}%) ont une LICENSE`);
    }
    // Positive insights
    if (healthScores.excellent > total * 0.5) {
        insights.push(`✅ ${healthScores.excellent} serveurs (${calculatePercentage(healthScores.excellent, total)}%) ont un EXCELLENT score de santé (≥80%)`);
    }
    if (activity.lastCommitLessThan1Month > total * 0.3) {
        insights.push(`🚀 ${activity.lastCommitLessThan1Month} serveurs sont ACTIFS (commit dans le dernier mois)`);
    }
    // Popularity insights
...
function printConsoleReport(report) {
    ...
    console.log(chalk_1.default.cyan.bold('   🔍 ANALYSE BASE DE DONNÉES MCP SERVERS'));
    console.log(chalk_1.default.gray(`   Date: ${report.timestamp}`));
    console.log(chalk_1.default.cyan.bold('═'.repeat(70)));
    // Configuration Statistics
    console.log('\n' + chalk_1.default.yellow.bold('📊 STATISTIQUES GÉNÉRALES'));
    const generalTable = new cli_table3_1.default({
        head: [chalk_1.default.white('Métrique'), chalk_1.default.white('Valeur')],
        style: { head: [], border: ['gray'] },
    });
    generalTable.push(['Total Serveurs', chalk_1.default.green(report.configStats.totalServers.toString())], ['Avec GitHub Info', `${report.configStats.withGithubInfo} (${calculatePercentage(report.configStats.withGithubInfo, report.configStats.totalServers)}%)`], ['Avec Outils (Tools)', `${report.configStats.withTools} serveurs / ${getAllTools().length} tools`]);
    console.log(generalTable.toString());
    // Configurations
    console.log('\n' + chalk_1.default.yellow.bold('🔧 CONFIGURATIONS D\'INSTALLATION'));
    const configTable = new cli_table3_1.default({
        head: [chalk_1.default.white('Type Config'), chalk_1.default.white('Count'), chalk_1.default.white('%')],
        style: { head: [], border: ['gray'] },
    });
    configTable.push(['NPM Config', report.configStats.withNpmConfig.toString(), chalk_1.default.red(`${report.configStats.npmConfigPercentage}%`)], ['Docker Config', report.configStats.withDockerConfig.toString(), chalk_1.default.red(`${report.configStats.dockerConfigPercentage}%`)], ['Sans Config', chalk_1.default.red(report.configStats.withNoConfig.toString()), chalk_1.default.red.bold(`${report.configStats.noConfigPercentage}%`)]);
    console.log(configTable.toString());
    // Health Scores
    console.log('\n' + chalk_1.default.yellow.bold('⭐ FIABILITÉ GITHUB (Health Score 0-100)'));
    const healthTable = new cli_table3_1.default({
        head: [chalk_1.default.white('Catégorie'), chalk_1.default.white('Seuil'), chalk_1.default.white('Count'), chalk_1.default.white('%')],
        style: { head: [], border: ['gray'] },
    });
    const totalWithGithub = report.configStats.withGithubInfo;
    healthTable.push([chalk_1.default.green('Excellent'), '≥80%', chalk_1.default.green(report.healthScoreDistribution.excellent.toString()), chalk_1.default.green(`${calculatePercentage(report.healthScoreDistribution.excellent, totalWithGithub)}%`)], [chalk_1.default.blue('Bon'), '≥60%', report.healthScoreDistribution.good.toString(), `${calculatePercentage(report.healthScoreDistribution.good, totalWithGithub)}%`], [chalk_1.default.yellow('Moyen'), '≥40%', report.healthScoreDistribution.medium.toString(), `${calculatePercentage(report.healthScoreDistribution.medium, totalWithGithub)}%`], [chalk_1.default.red('Faible'), '<40%', chalk_1.default.red(report.healthScoreDistribution.poor.toString()), chalk_1.default.red(`${calculatePercentage(report.healthScoreDistribution.poor, totalWithGithub)}%`)], [chalk_1.default.gray('Inconnu'), 'null', chalk_1.default.gray(report.healthScoreDistribution.unknown.toString()), chalk_1.default.gray(`${calculatePercentage(report.healthScoreDistribution.unknown, totalWithGithub)}%`)]);
    console.log(healthTable.toString());
    // Activity Metrics
    console.log('\n' + chalk_1.default.yellow.bold('🚀 ACTIVITÉ RÉCENTE'));
    const activityTable = new cli_table3_1.default({
        head: [chalk_1.default.white('Métrique'), chalk_1.default.white('Valeur')],
        style: { head: [], border: ['gray'] },
    });
    activityTable.push(['Commit Frequency (moy.)', `${report.activityMetrics.avgCommitFrequency} commits/30j`], ['Commit Frequency (médiane)', `${report.activityMetrics.medianCommitFrequency} commits/30j`], ['Dernier commit < 1 mois', chalk_1.default.green(`${report.activityMetrics.lastCommitLessThan1Month} serveurs`)], ['Dernier commit < 3 mois', `${report.activityMetrics.lastCommitLessThan3Months} serveurs`], ['Dernier commit < 6 mois', chalk_1.default.yellow(`${report.activityMetrics.lastCommitLessThan6Months} serveurs`)], ['Dernier commit > 6 mois', chalk_1.default.red(`${report.activityMetrics.lastCommitMoreThan6Months} serveurs`)], ['Sans données', chalk_1.default.gray(`${report.activityMetrics.noLastCommitData} serveurs`)]);
    console.log(activityTable.toString());
    // Popularity Metrics
    console.log('\n' + chalk_1.default.yellow.bold('🌟 POPULARITÉ'));
    const popularityTable = new cli_table3_1.default({
        head: [chalk_1.default.white('Métrique'), chalk_1.default.white('Moyenne'), chalk_1.default.white('Médiane')],
        style: { head: [], border: ['gray'] },
    });
    popularityTable.push(['GitHub Stars', report.popularityMetrics.avgStars.toString(), report.popularityMetrics.medianStars.toString()], ['GitHub Forks', report.popularityMetrics.avgForks.toString(), report.popularityMetrics.medianForks.toString()], ['GitHub Watchers', report.popularityMetrics.avgWatchers.toString(), report.popularityMetrics.medianWatchers.toString()], ['Contributors', report.popularityMetrics.avgContributors.toString(), report.popularityMetrics.medianContributors.toString()]);
    console.log(popularityTable.toString());
    // Stars Distribution
    console.log('\n' + chalk_1.default.yellow.bold('📊 DISTRIBUTION DES STARS'));
    const starsTable = new cli_table3_1.default({
        head: [chalk_1.default.white('Tranche'), chalk_1.default.white('Count'), chalk_1.default.white('%')],
        style: { head: [], border: ['gray'] },
    });
    ...
    // Top 10
    console.log('\n' + chalk_1.default.yellow.bold('🏆 TOP 10 SERVEURS PAR STARS'));
    const topTable = new cli_table3_1.default({
        head: [chalk_1.default.white('#'), chalk_1.default.white('Nom'), chalk_1.default.white('Stars'), chalk_1.default.white('GitHub URL')],
        style: { head: [], border: ['gray'] },
    });
    report.popularityMetrics.top10ByStars.forEach((server, idx) => {
        topTable.push([
            (idx + 1).toString(),
            server.name,
            chalk_1.default.yellow(server.stars.toLocaleString()),
            server.githubUrl || 'N/A'
        ]);
    });
    console.log(topTable.toString());
    // Quality Metrics
    console.log('\n' + chalk_1.default.yellow.bold('✅ QUALITÉ DES PROJETS'));
    const qualityTable = new cli_table3_1.default({
        head: [chalk_1.default.white('Indicateur'), chalk_1.default.white('Count'), chalk_1.default.white('%')],
        style: { head: [], border: ['gray'] },
    });
    qualityTable.push(['Has README', report.qualityMetrics.hasReadme.toString(), `${calculatePercentage(report.qualityMetrics.hasReadme, totalWithGithub)}%`], ['Has LICENSE', report.qualityMetrics.hasLicense.toString(), `${calculatePercentage(report.qualityMetrics.hasLicense, totalWithGithub)}%`], ['Has CONTRIBUTING', report.qualityMetrics.hasContributing.toString(), `${calculatePercentage(report.qualityMetrics.hasContributing, totalWithGithub)}%`], ['Has CODE_OF_CONDUCT', report.qualityMetrics.hasCodeOfConduct.toString(), `${calculatePercentage(report.qualityMetrics.hasCodeOfConduct, totalWithGithub)}%`], ['', '', ''], [chalk_1.default.red('Archived'), chalk_1.default.red(report.qualityMetrics.isArchived.toString()), chalk_1.default.red(`${calculatePercentage(report.qualityMetrics.isArchived, totalWithGithub)}%`)], [chalk_1.default.red('Disabled'), chalk_1.default.red(report.qualityMetrics.isDisabled.toString()), chalk_1.default.red(`${calculatePercentage(report.qualityMetrics.isDisabled, totalWithGithub)}%`)], [chalk_1.default.yellow('Fork'), report.qualityMetrics.isFork.toString(), `${calculatePercentage(report.qualityMetrics.isFork, totalWithGithub)}%`]);
    console.log(qualityTable.toString());
    // Completeness Report (Top 20)
    console.log('\n' + chalk_1.default.yellow.bold('📈 COMPLÉTUDE DES DONNÉES (Top 20)'));
    const completenessTable = new cli_table3_1.default({
        head: [chalk_1.default.white('Table'), chalk_1.default.white('Champ'), chalk_1.default.white('Présent'), chalk_1.default.white('Total'), chalk_1.default.white('%')],
        style: { head: [], border: ['gray'] },
    });
    report.completenessReports.slice(0, 20).forEach((item) => {
        const color = item.percentage >= 80 ? chalk_1.default.green : item.percentage >= 50 ? chalk_1.default.yellow : chalk_1.default.red;
    ...
    // Insights
    console.log('\n' + chalk_1.default.yellow.bold('💡 INSIGHTS CLÉS'));
    report.insights.forEach((insight) => {
        console.log(`  ${insight}`);
    });
    console.log('\n' + chalk_1.default.cyan.bold('═'.repeat(70)));
    console.log(chalk_1.default.green.bold('✅ Analyse terminée !'));
    console.log(chalk_1.default.cyan.bold('═'.repeat(70)) + '\n');
}
/**
...
function main() {
    ...
    try {
        ...
        const insights = generateInsights(configStats, healthScores, activityMetrics, popularityMetrics, qualityMetrics);
        // Build report
        const report = {
